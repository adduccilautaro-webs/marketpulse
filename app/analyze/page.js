...
export default function AnalyzePage() {
  ...
  const verdictColor = verdict === 'COMPRAR' ? 'var(--up)' : verdict === 'VENDER' ? 'var(--down)' : verdict === 'NEUTRAL' ? 'var(--neutral)' : 'var(--muted)'
  const verdictBg = verdict === 'COMPRAR' ? 'var(--up-dim)' : verdict === 'VENDER' ? 'var(--down-dim)' : verdict === 'NEUTRAL' ? 'var(--neutral-dim)' : 'var(--surface)'
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '3rem 2rem' }}>
        ...
        <div style={{ marginBottom: '2.5rem' }}>
          ...
          <p style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.8rem', color: 'var(--muted)', letterSpacing: '0.05em' }}>
            Fundamental · Técnico · Noticias · Veredicto de inversión
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: '1rem' }}>
          <button
            onClick={() => setLang('es')}
            style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.72rem', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', background: lang === 'es' ? 'var(--accent-dim)' : 'var(--surface)', color: lang === 'es' ? 'var(--accent)' : 'var(--muted)', border: lang === 'es' ? '1px solid rgba(79,195,247,0.3)' : '1px solid var(--border)', padding: '5px 14px', borderRadius: 2, cursor: 'pointer' }}
          >Español</button>
          <button
            onClick={() => setLang('en')}
            style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.72rem', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', background: lang === 'en' ? 'var(--accent-dim)' : 'var(--surface)', color: lang === 'en' ? 'var(--accent)' : 'var(--muted)', border: lang === 'en' ? '1px solid rgba(79,195,247,0.3)' : '1px solid var(--border)', padding: '5px 14px', borderRadius: 2, cursor: 'pointer' }}
          >English</button>
        </div>
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--muted)', fontFamily: "'DM Mono', monospace", fontSize: '0.85rem' }}>
            ...
            Buscando datos y generando análisis con IA...
          </div>
        )}
        ...
        {result && result.analysis && (
          <div>
            ...
            <Section title="Análisis Fundamental" content={result.analysis.fundamental} />
            <Section title="Análisis Técnico" content={result.analysis.technical} />

            {result.analysis.risks && (
              <Section title="Riesgos Principales" content={result.analysis.risks} color="var(--down)" />
            )}

            {result.news && result.news.length > 0 && (
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: '1.5rem', marginBottom: '1rem' }}>
                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.7rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '1rem' }}>
                  Noticias Recientes
                </div>
            ...
            {result.analysis.summary && (
              <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderLeft: '3px solid ' + verdictColor, padding: '1.5rem', marginBottom: '1rem' }}>
                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.7rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '0.75rem' }}>Conclusión</div>
